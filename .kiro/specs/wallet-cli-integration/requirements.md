...
6. 對 TRON 呼叫 `signTransaction(payload)` 時，應傳入 TRON 未簽交易 JSON，並以 `JSON.stringify(data.signed)` 回傳完整已簽交易，維持既有 `TronSigner` 慣例。
...
7. agent-wallet 系統應匯出 `WalletCliAdapter`、`WalletCliClient`、`WalletCliConfigResolver`、secret-provider 契約、dependency options 及相關錯誤與資料型別。
...
7. 本次不應將外部 signer registry 擴充為動態 params schema registry；此 P2 架構議題應明確延後，且不得阻塞 P0/P1 的安全與 EVM 相容修正。
...
3. TRON `signTransaction` 應回傳 `JSON.stringify(data.signed)`，其中 `signature[]` 與既有 `TronSigner` 相容；EVM `signTransaction` 應回傳 wallet-cli `data.signed.raw` 的 serialized transaction。
...
6. 既有 `raw_secret`、Privy、TRON-only optional integration 與公開 Wallet/x402 行為應具回歸測試，且 P2 registry schema 重構不得混入本次變更。
...
| 需求 5  | 錯誤階層與脫敏分類                                 |
| 需求 6  | 既有 builder registry；P2 明確延後                 |
