...
function setupRawSecretDir(): string {
...
function cloneDir(src: string, prefix: string): string {
  ...
  // Copy config file only (no secret files needed for raw_secret)
  cpSync(join(src, 'wallets_config.json'), join(dir, 'wallets_config.json'))
  return dir
}
...
beforeAll(() => {
  rawSecretTemplateDir = setupRawSecretDir()
})

beforeEach(() => {
  secretsDir = cloneDir(rawSecretTemplateDir, 'agent-wallet-registry-test-')
  resetWalletEnv()
  vi.restoreAllMocks()
})
...
afterAll(() => {
  if (rawSecretTemplateDir) {
    rmSync(rawSecretTemplateDir, { recursive: true, force: true })
  }
})
...
describe('ConfigWalletProvider', () => {
  ...
  it('persists setActive', () => {
    ...
    const reloaded = new ConfigWalletProvider(secretsDir, {})
  ...
  it('throws on invalid config instead of treating it as empty', () => {
    ...
    expect(() => new ConfigWalletProvider(secretsDir, {})).toThrow()
  })
})
...
describe('wallet builder contracts', () => {
  it('passes the full network string to adapters after family routing', async () => {
    ...
    const wallet = await createAdapter(conf, secretsDir, 'eip155:8453')
