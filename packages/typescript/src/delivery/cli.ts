...
async function getVerifiedPassword(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    io.print('✖ Wrong password, please try again.')
    const retryPw = await io.prompt('Master password', { password: true })
...
async function promptWalletId(io: CliIO, defaultValue: string, provider?: ConfigWalletProvider): Promise<string> {
  while (true) {
...
export async function cmdStart(
  ...
  if (wtype === WalletType.LOCAL_SECURE) {
    ...
    const targetName = opts?.walletId ?? (await promptWalletId(io, 'default', provider))
    let pw: string
    ...
    const targetName = opts?.walletId ?? (await promptWalletId(io, 'raw_wallet', provider))
...
export async function cmdAdd(
  ...
  const targetName = opts?.walletId ?? (await promptWalletId(io, 'wallet', provider))

  if (wtype === WalletType.LOCAL_SECURE) {
    const { pw, kvStore } = await getVerifiedPassword(dir, io, { explicit: opts?.password, provider })
