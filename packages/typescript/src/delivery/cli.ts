...
export async function cmdList(dir: string, io: CliIO): Promise<void> {
  ...
  for (const [wid, conf] of Object.entries(config.wallets)) {
    ...
    io.print(`${marker}${wid.padEnd(20)} ${conf.type.padEnd(15)} ${conf.address ?? '\u2014'}`)
  }
}
...
export async function cmdSignTx(wallet: string | undefined, payload: string, dir: string, io: CliIO): Promise<void> {
...
export async function cmdSignMsg(wallet: string | undefined, message: string, dir: string, io: CliIO): Promise<void> {
  const walletId = resolveWalletId(wallet, dir, io)
  const pw = await getPassword(io)
...
export async function cmdSignTypedData(wallet: string | undefined, data: string, dir: string, io: CliIO): Promise<void> {
  const walletId = resolveWalletId(wallet, dir, io)
  const pw = await getPassword(io)
  ...
  try {
    switch (command) {
      ...
      case 'sign':
        ...
        switch (subcommand) {
          case 'tx':
            await cmdSignTx((options.wallet ?? options.w) as string, (options.payload ?? options.p) as string, dir, cliIO)
            break
          case 'msg':
            await cmdSignMsg((options.wallet ?? options.w) as string, (options.message ?? options.m) as string, dir, cliIO)
            break
          case 'typed-data':
            await cmdSignTypedData((options.wallet ?? options.w) as string, options.data as string, dir, cliIO)
            break
          default:
