...
export async function cmdAdd(
  ...
  if (!opts?.walletType) {
    io.print(
      `Wallet type required. Usage: agent-wallet add <${Object.values(WalletType).join("|")}>`,
    );
    throw new CliExit(1);
  }
  const wtype = await selectWalletType(opts.walletType, io);
...
export async function cmdChangePassword(
  ...
  const newPw = await io.prompt("New password (min 8 chars, upper+lower+digit+special)", {
    password: true,
  });
