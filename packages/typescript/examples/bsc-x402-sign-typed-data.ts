 ...
 *
 * Config-backed wallets also work through the normal AGENT_WALLET_DIR lookup.
 */
...
async function main() {
  ...
  const sig1 = await wallet.signTypedData(PAYMENT_PERMIT)
  ...
  const sig2 = await wallet.signTypedData(STANDARD_TYPED_DATA)
