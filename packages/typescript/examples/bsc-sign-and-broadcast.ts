...
const MESSAGE_TYPED_DATA = {
  ...
  primaryType: 'Message',
  domain: { name: 'AgentWallet', version: '1', chainId: CHAIN_ID },
...
async function main() {
  ...
  const signed = await wallet.signTransaction(tx as unknown as Record<string, unknown>)
  if (signed.family !== 'evm') throw new Error('Expected an EVM signed transaction')
  const signedRawHex = signed.rawTransaction
